...
const Recording = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const please = copyData.map(async (el) => {
        if (el.image !== "") {
          ...
          if (!el.image.includes("https")) {
            ...
            let please = await loloca
              .child(`book/${nameT}`)
              .putString(el.image, "data_url");
      ...
      const filterObj = numFruits.filter(
        (el) => (el.text !== "" || el.image !== "") && el.page !== ""
      );
      ...
      const objobj = {
        ...
        id: isbn,
        creationData:
          bookInfo.creationData === null
            ? moment().format("YYYYMMDD")
            : bookInfo.creationData,
        startDate:
          typeof bookInfo.startDate === "string"
            ? bookInfo.startDate
            : moment(bookInfo.startDate).format("YYYY.MM.DD"),
        endDate:
          typeof bookInfo.endDate === "string"
            ? bookInfo.endDate
            : moment(bookInfo.endDate).format("YYYY.MM.DD"),
        bookmark: filterObj,
      };
        ...
        <>
          {Object.keys(bookInfo).length !== 0 && (
            <>
                ...
                <BookInfoTop bookInfo={bookInfo}>
                  <StarRating>
                    {[1, 2, 3, 4, 5].map((el) => (
                      <div
                        key={el}
                        className={bookInfo.star >= el ? "onStar" : ""}
                        onClick={() => setBookInfo({ ...bookInfo, star: el })}
                      >
                        <AiFillStar />
                      </div>
                    ))}
                    ...
                    <DateBox>
                      ...
                      <div className="pickDate">
                        ...
                        <DatePicker
                          selected={
                            typeof bookInfo.endDate === "string"
                              ? new Date(bookInfo.endDate)
                              : bookInfo.endDate
                          }
                          dateFormat="yyyy-MM-dd"
                          onFocus={(e) => e.target.blur()}
                          onChange={(date) =>
                            setBookInfo({ ...bookInfo, endDate: date })
                          }
                        />
                      </div>
                    </DateBox>
              ...
              <div className="btn uploadBtn" onClick={onSubmit}>
                {state === "write" ? "독서 기록하기" : "수정하기"}
              </div>
            </>
          )}
        </>
